...
public class WMR200ServiceImpl extends NotificationBroadcasterSupport implements WMR200Service, Runnable {
	...
	/** The logger. */
	private static Logger logger = Logger.getLogger(WMR200ServiceImpl.class
			.getName());
	...
	public void start() {
		...
		if (!pooling){
			Pooling(500, TimeUnit.MILLISECONDS);
		}
	}
	...
	// return tag;
	// }
	// logger.debug(">>wmr200 " + wmr200ID + " is pooling .");
	// return null;
	//
	// }
	...
	// if (timer == null || timer.isShutdown()) {
	// logger.debug(">>wmr200 " + wmr200ID + " is not pooling!");
	// return tags;
	// }
	...
	public void run() {
		...
		try {
			...
			if (bigBuffer.size() >= 2) {
				...
				if (bigBuffer.size() == (bigBuffer.get(1) & 0xff)) {
	...
	public void readThermometerD7(List<Byte> bigBuffer)
		...
		// m_lastMeasurementTimes.put(WMR200MapKey.THERMOMETER, date);
		old_m_allData = m_allData;
		old_m_lastMeasurementTimes = m_lastMeasurementTimes;
