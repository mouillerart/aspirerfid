...
public class Pachube2 extends Pachube.Pachube {

	
	public final static String APIv2URL="http://api.pachube.com/v2";
	...
	/**
	 * API key for your user account on Pachube2
	 */
	 ...
	 * @param APIKEY
	 */
	public Pachube2(String APIKEY) {
		super(APIKEY);
		this.API_KEY = APIKEY;
		this.client = new HttpClient("api.pachube.com");
	}
	...
	public Feed getFeed(int feed) throws PachubeException {
		HttpRequest hr = new HttpRequest(APIv2URL+"/feeds/"
				+ feed + ".xml");
		...
		if (g.getHeaderItem("Status").equals("HTTP/1.1 200 OK")) {
			return Pachube2Factory.toFeed(this, g.getBody());
		} else {
	...
	public Double[] getDatastreamHistory(int feed, int datastream) {
		HttpRequest hr = new HttpRequest(APIv2URL+"/feeds/" + feed + "/datastreams/" + datastream + "/history.csv");
		hr.setMethod(HttpMethod.GET);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
	...
	public String[] getDatastreamArchive(int feed, int datastream) {
		HttpRequest hr = new HttpRequest(APIv2URL+"/feeds/" + feed
	...
	public String createTrigger(Trigger t) throws PachubeException {
		HttpRequest hr = new HttpRequest(APIv2URL+"/triggers/");
		hr.setMethod(HttpMethod.POST);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
	...
	public Trigger getTrigger(int id) throws PachubeException {
		HttpRequest hr = new HttpRequest(APIv2URL+"/triggers//"+id+".xml");
		hr.setMethod(HttpMethod.GET);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
		HttpResponse h = client.send(hr);
		
		return Pachube2Factory.toTrigger(h.getBody())[0];
	...
	public Trigger[] getTriggers() throws PachubeException {
		...
		return Pachube2Factory.toTrigger(h.getBody());
	...
	public HttpResponse deleteTrigger(int id){
		HttpRequest hr = new HttpRequest(APIv2URL+"/triggers//"+id);
		hr.setMethod(HttpMethod.DELETE);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
		return client.send(hr);
	...
	/**
	 * Gets a Pachube2 graph of the datastream
	 * 
	 * @param feedID
	...
	public String showGraph(int feedID, int streamID, int width, int height,
		...
		return APIv2URL+"/feeds/" + feedID + "/datastreams/"
